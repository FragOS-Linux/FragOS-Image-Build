...
WEBSITE="https://fragos.github.io"
KERNEL_PACKAGE="linux"
PACKAGES=$(grep -vE '^\s*#|^\s*$' packages.conf | tr '\n' ' ')
SERVICES=$(cat services-to-enable.conf | tr -d '"' | tr -d '\\' | tr '\n' ' ')
USER_SERVICES=$(cat services-to-enable-user.conf | tr -d '"' | tr -d '\\' | tr '\n' ' ')
FILES_TO_DELETE="\
...
echo FRAGOS SYSTEM BUILDER...

echo STAGE 1: Downloading rootfs from pacstrap...

pacstrap -K -C rootfs/etc/pacman.conf ${BUILD_PATH}

echo STAGE 2: Configuring rootfs...
